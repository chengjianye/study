  ...
  "pages": [
    ...
    },
    {
      ...
      "headers": [
        {
          "level": 3,
          "title": "DIFF 算法在执行时有三个维度，分别是 Tree DIFF、Component DIFF 和 Element DIFF，执行时按顺序依次执行，它们的差异仅仅因为 DIFF 粒度不同、执行先后顺序不同。",
          "slug": "diff-算法在执行时有三个维度，分别是-tree-diff、component-diff-和-element-diff，执行时按顺序依次执行，它们的差异仅仅因为-diff-粒度不同、执行先后顺序不同。"
        },
        {
          "level": 3,
          "title": "Tree DIFF 是对树的每一层进行遍历，如果某组件不存在了，则会直接销毁。如图所示，左边是旧属，右边是新属，第一层是 R 组件，一模一样，不会发生变化；第二层进入 Component DIFF，同一类型组件继续比较下去，发现 A 组件没有，所以直接删掉 A、B、C 组件；继续第三层，重新创建 A、B、C 组件。",
          "slug": "tree-diff-是对树的每一层进行遍历，如果某组件不存在了，则会直接销毁。如图所示，左边是旧属，右边是新属，第一层是-r-组件，一模一样，不会发生变化；第二层进入-component-diff，同一类型组件继续比较下去，发现-a-组件没有，所以直接删掉-a、b、c-组件；继续第三层，重新创建-a、b、c-组件。"
        },
        {
          "level": 3,
          "title": "如图所示，第一层遍历完，进行第二层遍历时，D 和 G 组件是不同类型的组件，不同类型组件直接进行替换，将 D 删掉，再将 G 重建",
          "slug": "如图所示，第一层遍历完，进行第二层遍历时，d-和-g-组件是不同类型的组件，不同类型组件直接进行替换，将-d-删掉，再将-g-重建"
        },
        {
          "level": 3,
          "title": "作用",
        ...
        {
          "level": 3,
          "title": "当数据发生变化时，vue 是怎么更新节点的？",
          "slug": "当数据发生变化时，vue-是怎么更新节点的？"
        },
        {
          "level": 3,
          "title": "virtual DOM 和真实 DOM 的区别？",
          "slug": "virtual-dom-和真实-dom-的区别？"
        },
        {
          "level": 3,
          "title": "diff 的比较方式?",
        ...
        {
          "level": 3,
          "title": "render 执行的结果得到的不是真正的 DOM 节点.结果仅仅是轻量级的 JavaScript 对象, 我们称之为 virtual DOM.",
          "slug": "render-执行的结果得到的不是真正的-dom-节点-结果仅仅是轻量级的-javascript-对象-我们称之为-virtual-dom"
        },
        {
          "level": 3,
          "title": "React 工作流程",
        ...
        {
          "level": 3,
          "title": "调和：将 Virtual DOM 树转换成 actual DOM 树的最少操作的过程 称为 调和 。",
          "slug": "调和：将-virtual-dom-树转换成-actual-dom-树的最少操作的过程-称为-调和-。"
        },
        {
          "level": 3,
          "title": "React diff 算法：diff 算法是调和的具体实现。",
          "slug": "react-diff-算法：diff-算法是调和的具体实现。"
        },
        {
          "level": 3,
          "title": "diff 策略",
        ...
        {
          "level": 3,
          "title": "如果 DOM 节点出现了跨层级操作,diff 会咋办呢？",
          "slug": "如果-dom-节点出现了跨层级操作-diff-会咋办呢？"
        },
        {
          "level": 3,
          "title": "重点说下移动的逻辑：",
